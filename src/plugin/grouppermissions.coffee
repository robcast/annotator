...
class Annotator.Plugin.GroupPermissions extends Annotator.Plugin
  ...
  options:
    ...
    # Display selection of groups in the Editor.
  ...
  updateGroupPermissionsField: (action, field, annotation) =>
    ...
    # Do not show field if current user is not admin.
    if not (this.authorize('admin', annotation) and @groups?.length > 0)
      field.hide()
      return
